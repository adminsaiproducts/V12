...
**GCS Export/Import UIでのパス検証エラー**:
- Cloud Console UIでFirestoreインポートを実行しようとすると、パスにコロン（:）が含まれる場合バリデーションエラー
- Firestoreエクスポートのフォルダ名はタイムスタンプにコロンを含む（例: `2025-12-29T14:44:37_59262`）
- 解決: gcloud CLIを使用するか、スクリプトベースの移行（`migrate-firestore-data.cjs`）を使用
